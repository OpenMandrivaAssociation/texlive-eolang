Name:		texlive-eolang
Version:	69274
Release:	1
Summary:	Formulas and graphs for the EO programming language
Group:		Publishing
URL:		https://www.ctan.org/tex-archive/macros/latex/contrib/eolang
License:	mit
Source0:	http://mirrors.ctan.org/systems/texlive/tlnet/archive/eolang.r%{version}.tar.xz
Source1:	http://mirrors.ctan.org/systems/texlive/tlnet/archive/eolang.doc.r%{version}.tar.xz
Source2:	http://mirrors.ctan.org/systems/texlive/tlnet/archive/eolang.source.r%{version}.tar.xz
BuildArch:	noarch
BuildRequires:	texlive-tlpkg
Requires(pre):	texlive-tlpkg
Requires(post):	texlive-kpathsea

%description
This LaTeX package helps you write [?] -calculus formulas and
SODG graphs for the EO programming language.

%prep
%setup -c -a1 -a2
%autopatch -p1

%build

%install
rm -rf tlpkg
mkdir -p %{buildroot}%{_texmfdistdir}
cp -a * %{buildroot}%{_texmfdistdir}

%files
%doc %{_texmfdistdir}/source/latex/eolang
%{_texmfdistdir}/tex/latex/eolang
%doc %{_texmfdistdir}/doc/latex/eolang

%post -p %{_sbindir}/texlive.post

%postun
[ "$1" -eq 0 ] && %{_sbindir}/texlive.post
